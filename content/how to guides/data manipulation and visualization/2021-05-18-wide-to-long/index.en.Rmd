...
Structured datasets tend to come in one of two formats, wide or long. In the wide format, each row contains data related to a particular subject or event where each column contains a different variable taken from that subject. On the other hand, long data format is when all variables are placed in a column. Some R functions work best with wide or long data so it is important to know how to reshape data.  This guide explains how to convert a wide format dataset to long format using tidyverse. The data are from the AMCP package and were created to run a two-way repeated measures ANOVA.
...
We could also convert variable to factor with the `convert_as_factor()` function from the rstatix package. We can pipe each of the steps above and add the `convert_as_factor()` function at the end setting the columns we would like to conver.
