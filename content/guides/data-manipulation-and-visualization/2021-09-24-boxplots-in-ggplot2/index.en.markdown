...
### Footnotes
[^1]: The data presented here are from a repeated measures ANOVA design. One of the central assumptions for repeated measures ANOVA is sphericity. Sphericity is the condition in which the variance of the differences between all possible conditions is equal. It's analogous to the assumption of homogeneity of variance in n-way ANOVA. While it is possible to correct for violations of sphericity with the the Greenhouse-Geisser and Huyn-Feldt procedures, these corrections complicate performing multiple comparisons. The idea here is that the correction applied to the entire data set may be inappropriate for portions of the data such as when performing multiple comparisons. While multiple comparison were performed here for illustrative purposes, they may be inappropriate for your data if a sphericity correction was applied.
